  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950">
      <div className="w-full max-w-md p-8 bg-white dark:bg-gray-900 rounded-xl shadow-lg">
                  ...
                  <form key="credentials" onSubmit={handleCredentialsLogin} className="space-y-3">
                    <div className="px-1 py-2">
                      ...
                      <input
                        ...
                        required
                        className="w-full rounded-md border border-gray-300 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                      />
                    </div>
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full rounded-md bg-indigo-600 px-4 py-3 text-sm font-medium text-white hover:bg-indigo-700 transition-colors disabled:opacity-50"
                    >
                      {loading ? "登录中..." : "登录"}
                    </button>
                  </form>
        ...
        {!providers && (
          <p className="text-center text-gray-400 text-sm">加载中...</p>
        )}
      </div>
    </main>
  );
...
export default function LoginPage() {
  return (
    <Suspense fallback={<main className="min-h-screen flex items-center justify-center bg-gray-50"><p className="text-gray-400">加载中...</p></main>}>
      <LoginForm />
    </Suspense>
  );
}
